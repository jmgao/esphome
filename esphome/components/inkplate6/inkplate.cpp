  ...
  if (!this->greyscale_) {
    ...
    for (int i = 0; i < 8; ++i) {
      for (uint32_t j = 0; j < 256; ++j) {
        ...
        this->glut_[i * 256 + j] = ((z & B00000011) << 4) | (((z & B00001100) >> 2) << 18) |
                                   (((z & B00010000) >> 4) << 23) | (((z & B11100000) >> 5) << 25);
        z = ((waveform3Bit[j & 0x07][i] << 2) | (waveform3Bit[(j >> 4) & 0x07][i])) << 4;
        this->glut2_[i * 256 + j] = ((z & B00000011) << 4) | (((z & B00001100) >> 2) << 18) |
                                    (((z & B00010000) >> 4) << 23) | (((z & B11100000) >> 5) << 25);
      }
    }
  }
...
void Inkplate6::display3b_() {
  ...
  for (int k = 0; k < 8; k++) {
    ...
    for (int i = 0; i < this->get_height_internal(); i++) {
      ...
      GPIO.out_w1ts = (this->glut2_[k * 256 + this->buffer_[--pos]] | this->glut_[k * 256 + this->buffer_[--pos]]) |
                      (1 << this->cl_pin_->get_pin());
      GPIO.out_w1tc = get_data_pin_mask_() | (1 << this->cl_pin_->get_pin());
